...
public class DetailMyWorld {
    ...
    public static Set<Integer> bfs(ArrayList<ArrayList<Integer>> world, int i, int j) {
        ...
        Queue<Integer> row = new LinkedBlockingQueue<>();
        Queue<Integer> col = new LinkedBlockingQueue<>();
        row.offer(i);
        col.offer(j);
        continent.add(world.get(i).get(j)); // optimisele code
        world.get(i).set(j, 0);
        ...
        while (!row.isEmpty()) {
            ...
            if (x != 0) {
                if (world.get(y).get(x - 1) != 0) {
                    col.offer(x - 1);
                    row.offer(y);
                    continent.add(world.get(y).get(x - 1)); // optimisele code
            ...
            if (y != 0) {
                if (world.get(y - 1).get(x) != 0) {
                    col.offer(x);
                    row.offer(y - 1);
                    continent.add(world.get(y - 1).get(x)); // optimisele code
